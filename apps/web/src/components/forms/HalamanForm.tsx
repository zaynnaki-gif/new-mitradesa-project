...
interface HalamanFormProps {
  mode: 'create' | 'edit';
  initialData?: Partial<HalamanFormData>;
  onSuccess: () => void;
  onCancel: () => void;
}
...
export function HalamanForm({ mode, initialData, onSuccess, onCancel }: HalamanFormProps) {
  ...
  const [form, setForm] = useState<HalamanFormData>({
    ...
    isMenu: initialData?.isMenu ?? false,
  ...
  const checkSlugAvailability = async (slug: string, excludeSlug?: string): Promise<boolean> => {
    ...
    try {
      const res = await fetch(`/api/halaman/slug/${slug}`);
  ...
  const handleChange = (field: keyof HalamanFormData, value: string | number | boolean) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        throw new Error(result.error?.message || 'Gagal menyimpan halaman');
      }
    } catch (err) {
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', paddingTop: '1.5rem' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
              ...
              />
              <span style={{ fontSize: '0.875rem' }}>Tampilkan di Menu</span>
            </label>
          </div>
        </div>

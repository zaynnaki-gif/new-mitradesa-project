...
/**
 * RefStatusPerkawinan Service - Marital Status Master Data
 */
export class RefStatusPerkawinanService {
  ...
  async findAll(options: { page?: number; limit?: number; search?: string; isAktif?: boolean } = {}) {
    ...
    const [data, total] = await Promise.all([
      prisma.refStatusPerkawinan.findMany({ where, skip, take: limit, orderBy: { kode: 'asc' } }),
      prisma.refStatusPerkawinan.count({ where }),
    ]);
  ...
  async update(kode: string, data: { nama?: string; isAktif?: boolean }, actorId?: bigint, actorIp?: string, actorAgent?: string) {
    const existing = await prisma.refStatusPerkawinan.findUnique({ where: { kode } });
    if (!existing) throw ApiError.notFound('Data tidak ditemukan');

    const result = await prisma.refStatusPerkawinan.update({ where: { kode }, data });
  ...
  async activate(kode: string, actorId?: bigint, actorIp?: string, actorAgent?: string) {
    ...
    const result = await prisma.refStatusPerkawinan.update({
      where: { kode },
      data: { isAktif: true },
    });
...
export const refStatusPerkawinanService = new RefStatusPerkawinanService();

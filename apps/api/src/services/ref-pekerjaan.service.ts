...
/**
 * RefPekerjaan Service - Occupation Master Data
 */
export class RefPekerjaanService {
  ...
  async findAll(options: { page?: number; limit?: number; search?: string; isAktif?: boolean; kategori?: string } = {}) {
    ...
    const [data, total] = await Promise.all([
      prisma.refPekerjaan.findMany({ where, skip, take: limit, orderBy: { nama: 'asc' } }),
      prisma.refPekerjaan.count({ where }),
    ]);
  ...
  async findByKode(kode: string) {
    return prisma.refPekerjaan.findUnique({ where: { kode } });
  }
  ...
  async create(data: { kode: string; nama: string; kategori?: string; isAktif?: boolean }, actorId?: bigint, actorIp?: string, actorAgent?: string) {
    const existing = await prisma.refPekerjaan.findUnique({ where: { kode: data.kode } });
    if (existing) throw ApiError.conflict('Kode sudah ada');

    const result = await prisma.refPekerjaan.create({
      data: { kode: data.kode, nama: data.nama, kategori: data.kategori, isAktif: data.isAktif ?? true },
    });

    await this.audit.log({
      entityType: 'ref_pekerjaan',
      entityId: result.id,
  ...
  async update(kode: string, data: { nama?: string; kategori?: string; isAktif?: boolean }, actorId?: bigint, actorIp?: string, actorAgent?: string) {
    const existing = await prisma.refPekerjaan.findUnique({ where: { kode } });
    if (!existing) throw ApiError.notFound('Data tidak ditemukan');

    const result = await prisma.refPekerjaan.update({ where: { kode }, data });
  ...
  async activate(kode: string, actorId?: bigint, actorIp?: string, actorAgent?: string) {
    ...
    const result = await prisma.refPekerjaan.update({
      where: { kode },
      data: { isAktif: true },
    });
...
export const refPekerjaanService = new RefPekerjaanService();

...
test.describe('Agenda E2E', () => {
  test('Buat Agenda and check public frontend', async ({ page }) => {
    // 1. Login
    ...
    await page.getByLabel('Admin navigation').getByRole('link', { name: /agenda/i }).click();

    // 3. Verify page
    await expect(page.getByRole('heading', { name: /Agenda/i })).toBeVisible({ timeout: 15000 });
  });
});

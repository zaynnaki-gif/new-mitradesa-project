...
test.describe('UMKM E2E', () => {
  test('Buat UMKM and check public frontend', async ({ page }) => {
    // 1. Login
    ...
    await page.getByLabel('Admin navigation').getByRole('link', { name: /umkm/i }).click();

    // 3. Verify page
    await expect(page.getByRole('heading', { name: /UMKM/i })).toBeVisible({ timeout: 15000 });
  });
});
